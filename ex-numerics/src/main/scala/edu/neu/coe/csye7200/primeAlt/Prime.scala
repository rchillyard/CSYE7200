...
abstract class BigIntLike(val x: BigInt) extends ScalaNumber {
  ...
  protected def validate: Boolean = {
    ...
    if (logger.isDebugEnabled && isPrime) logger.debug(s"$x is prime")
    isPrime
  }
}

...
@main def runGitHubAPI(): Unit = {
  ...
  Try(os.read(os.home / "github_token.txt")).transform(token => invokeAPI(token.trim), ex => showException(ex)) match {
    case Success(_) =>
      println("API invoked successfully")
    case Failure(ex) =>
      showException(ex): Unit
  }
}

...
object StockReader {
  ...
  def getPrice(symbol: String)(implicit resource: String): Future[Double] =
    MonadOps.flatten(Future(readPrice(symbol)))
  ...
  private def readPrice(symbol: String)(implicit resource: String): Try[Double] = {
    ...
    val woy = Try(readMatchingPrices.headOption)
    val wy = woy.map(_.getOrElse(throw new Exception(s"no entry matching $symbol")))

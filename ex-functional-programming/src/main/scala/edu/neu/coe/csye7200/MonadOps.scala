...
object MonadOps {
  ...
  def flatten[X](xoy: Try[Option[X]], t: => Throwable): Try[X] =
    for (xo <- xoy; x <- optionToTry(xo, t)) yield x

  def flatten[X](xoy: Try[Option[X]]): Try[X] =
    for (xo <- xoy; x <- optionToTry(xo)) yield x

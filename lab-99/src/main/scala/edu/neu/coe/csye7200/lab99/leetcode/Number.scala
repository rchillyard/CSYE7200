package edu.neu.coe.csye7200.lab99.leetcode
...
/**
  ...
  *
  * @param x digit
  * @param nextMostSignificantNumber the Number represented by the digits to the left of x.
  */
case class Number_Recursive(x: Int, nextMostSignificantNumber: Number) extends Number {
  /**
    * Apply method which converts this Number into a BigInt.
    *
    * @return a BigInt corresponding to the value of this Number.
    */
  override def apply(): BigInt = x + nextMostSignificantNumber() * BigInt(10)

  private lazy val render: String = this match {
...
object Number {
  ...
  def apply(xs: Seq[Int]): Number = xs match {
    case Nil => Zero
    case h :: t => Number_Recursive(h, Number(t))
  }

  def apply(x: Long): Number = Number(getDigits(x))

  def apply(x: BigInt): Number = Number(getDigits(x))
